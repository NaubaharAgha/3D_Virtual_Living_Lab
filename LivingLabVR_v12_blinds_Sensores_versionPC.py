...
lampara =  viz.add('art/flashlight.IVE')
lampara.setPosition(0,0.3,5)
lampara.setScale(0.006,0.006,0.006)
...
def EnterProximity(e):
	...
	if e.sensor == sensor_doorSuichbath:
		...
		texto.visible(viz.OFF) #Borra el texto que hubiera
		texto = viz.addText('Click para abrir/cerrar la puerta del baño',parent=viz.SCREEN) #Añade el texto del objeto
		texto.fontSize(100)
		texto.visible(viz.ON)
		sen = 3
	...
	if e.sensor == sensor_planta:
		...
		texto.visible(viz.OFF) #Borra el texto que hubiera
		texto = viz.addText('Planta',parent=viz.SCREEN, pos=[0.5,0.900,0],scale=[0.1,0.5,1], color = viz.BLUE) #Añade el texto del objeto
		texto.visible(viz.ON)
		sen = 4
		#vizact.onmousedown(viz.MOUSEBUTTON_LEFT, doorbath)
	...
	if e.sensor == sensor_RollupBED:
		...
		texto.visible(viz.OFF) #Borra el texto que hubiera
		texto = viz.addText('Click para abrir/cerrar la persiana de la habitación',parent=viz.SCREEN,pos=[0.1,0.900,0],scale=[0.5,0.5,1], color = viz.PURPLE) #Añade el texto del objeto
		#texto.visible(viz.ON)
		sen = 7
...
def click_mouse(): #CONTROL COMPU
	...
	if sen == 0:
		global texto
		texto.visible(viz.OFF) #Borra el texto que hubiera
		texto = viz.addText('Elija un elemento para realizar la acción',parent=viz.SCREEN, pos=[1,2,0],scale=[0.1,0.5,1], color = viz.BLUE) #Añade el texto del objeto
		texto.visible(viz.ON)
