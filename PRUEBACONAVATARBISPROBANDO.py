...
persona = viz.add('vcc_female.cfg')
persona.setPosition(1,0.58,6)
persona.state(2)

persona.setEuler(180,0,0)
...
def doorSCOpen():	
	...
	if(state_SV==0):
		...
		viz.MainView.addAction(vizact.goto([-3.5732383728027344, 2.407294273376465, -1.3862537145614624],viz.ABS_GLOBAL))
		if state_doorSC == 0 and state_doorSC_quiero == 1 : #ABRIR
			eule = [90,0,0]
			DoorSC_GEODE.setEuler(eule, viz.REL_LOCAL)
			state_doorSC = 1
...
#def doorHome(e): 
def doorHomeOpen():
	print 'Coordenadas :',viz.MainView.getPosition(viz.ABS_GLOBAL)
	...
	global state_SV 
	state_doorHome_quiero = 1	
	#viz.MainView.setPosition([3.7735934257507324, 2.407294273376465, 2.2873051166534424],viz.ABS_GLOBAL)
	viz.MainView.setEuler([88.15464782714844, 0.0, 0.0],viz.ABS_GLOBAL)	
	#persona.addAction( vizact.walkTo([6.50, 0.58, 2]) )
	#persona.addAction( vizact.walkTo([3.77, 0.58, 2.28],viz.ABS_GLOBAL) )
	
	viz.MainView.addAction(vizact.goto([3.7735934257507324, 2.407294273376465, 2.2873051166534424],viz.ABS_GLOBAL))
...
def doorbathOpen():
	...
	print 'vista Door_Baño', viz.MainView.getPosition(viz.ABS_GLOBAL)
	print 'vista Door_Baño_Euler' , viz.MainView.getEuler(viz.ABS_GLOBAL)
	
	
	viz.MainView.setEuler([-93.51321411132812, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([2.714972734451294, 2.407294273376465, 7.522972106933594],viz.ABS_GLOBAL))
...
def doorKitchenOpen ():
	...
	viz.MainView.setEuler([-171.30130004882812, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([0.8414586186408997, 2.407294273376465, 5.400276184082031],viz.ABS_GLOBAL))
	def openDoorKitchen ():
...
def doorSalonOpen ():
	...
	viz.MainView.setEuler([168.10745239257812, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([4.092425346374512, 2.407294273376465, 4.741787910461426],viz.ABS_GLOBAL))
	def openDoorSalon ():
	...
	persona.addAction( vizact.walkTo([5,0.58,1]) )
...
def RollupBedOpen ():
	...
	viz.MainView.setEuler([60.40805435180664, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([2.807915687561035, 2.407294273376465, 6.183692455291748],viz.ABS_GLOBAL))
...
def RollupSCOpen ():
	...
	viz.MainView.setEuler([-74.78972625732422, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([-4.8969316482543945, 2.407294273376465, 3.1122682094573975],viz.ABS_GLOBAL))
...
def Kitchen_lightONOFFOpen ():
	...
	viz.MainView.setEuler([-109.53791809082031, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([3.603083610534668, 2.407294273376465, 4.1396026611328125],viz.ABS_GLOBAL))
...
def Sala_lightONOFFOpen ():
	global state_SV
	state_SV = 0
	viz.MainView.setEuler([-66.82832336425781, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([6.390945911407471, 2.407294273376465, 4.5051164627075195],viz.ABS_GLOBAL))
	print 'vista SalonLuz', viz.MainView.getPosition(viz.ABS_GLOBAL)
	print 'vista SalonLuz_Euler' , viz.MainView.getEuler(viz.ABS_GLOBAL)
...
def Bathroom_lightONOFFOpen ():
	...
	global state_SV
	state_SV = 0
	viz.MainView.setEuler([-92.50753021240234, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([-2.0080931186676025, 2.407294273376465, 7.663171291351318],viz.ABS_GLOBAL))
...
def SC_lightONOFFOpen ():
	global state_SV
	state_SV = 1
	viz.MainView.setEuler([-19.274208068847656, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([-5.601597309112549, 2.407294273376465, -0.16091285645961761],viz.ABS_GLOBAL))
...
def RV_lightONOFFOpen ():
	global state_SV
	state_SV = 1
	viz.MainView.setEuler([30.27346420288086, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([-8.30616283416748, 2.407294273376465, 4.816455841064453],viz.ABS_GLOBAL))
...
def Casa_SC():
	...
	doorHomeOpen()
	viz.waitTime(1.5)
	viz.MainView.setEuler([82.54271697998047, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([9.105006217956543, 2.4000000953674316, 2.198660135269165],viz.ABS_GLOBAL))
	
	viz.waitTime(1.5)
	viz.MainView.setEuler([-110.50438690185547, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.MainView.addAction(vizact.goto([8.841052055358887, 2.4000000953674316, -0.42054590582847595],viz.ABS_GLOBAL))
	
	viz.waitTime(1.5)
	doorSCOpen()
	viz.waitTime(1.5)
	viz.MainView.addAction(vizact.goto([-5.458252429962158, 2.407294273376465, 1.1546337604522705],viz.ABS_GLOBAL))
	viz.waitTime(2)
	viz.MainView.setEuler([-30.44811248779297, 0.0, 0.0],viz.ABS_GLOBAL)
	
	
def SC_Casa():	
	global state_SV
	state_SV = 1
	viz.MainView.setPosition([-1.9530490636825562, 2.407294273376465, -0.8531053066253662],viz.ABS_GLOBAL)
	viz.MainView.setEuler([-89.4607925415039, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.waitTime(1)
	doorSCOpen()
	viz.waitTime(1)
	viz.MainView.setPosition([8.841052055358887, 2.4000000953674316, -0.42054590582847595],viz.ABS_GLOBAL)
	viz.MainView.setEuler([-110.50438690185547, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.waitTime(1)
	viz.MainView.setPosition([9.105006217956543, 2.4000000953674316, 2.198660135269165],viz.ABS_GLOBAL)
	viz.MainView.setEuler([82.54271697998047, 0.0, 0.0],viz.ABS_GLOBAL)
	viz.waitTime(1)
	doorHomeOpen()
